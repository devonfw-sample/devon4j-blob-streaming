...
The actual data is provided as an `InputStream` in the `data` part. The `InputStream` is provided by the link:core/src/main/java/com/devonfw/example/blobapp/binary/dataaccess/api/BinaryObjectEntity.java. The data is mapped as `java.sql.Blob`.
